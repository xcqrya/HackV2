...
const questions = [
    ...
    },
    {
        type: "list",
        name: "round",
        message: color("Auth Lu di ambil di Stage Brp"),
        prefix: `${color("[", "cyanBright")}XcBot${color("]", "cyanBright")}`,
        suffix: "=",
   choices: ["Stage 1", "Stage 2", "Stage 3"], filter: (value) => { return { "Stage 1": 1, "Stage 2": 2, "Stage 3": 3,
    ...
    },
    {
        type: "input",
        name: "interval",
        message: color("Masukin Delay TodðŸ—¿"),
        prefix: `${color("[", "cyanBright")}XcBot${color("]", "cyanBright")}`,
        suffix: "=",
        default: 1500,
        validate: function (input) {
...
const asciiText = figlet.textSync("Xcqrya HackV2", {
    font: 'Soft',
    ...
    setInterval(async function iStumble() {
        try {
            ...
            if (typeof data == "string" && data.includes("BANNED")) {
                ...
                const print = `\x1b[44m[Apa itu ban]\x1b[0m >> ` + [color(Username, "blueBright"), color(Country, "cyan"), color(Crowns, "greenBright"), color(SkillRating, "yellowBright")].join(" ! ");
                console.log(print);
            }
        } catch (error) {}
    }, Number(interval));
